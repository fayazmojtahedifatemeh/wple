...
if (!config) {
    ...
    if (url.includes('gianaworld')) {
      throw new Error('This site should not be using Puppeteer. Check scraper.ts');
    }
...
async function extractAymProduct($: any, url: string): Promise<ScrapedProduct> {
  ...
  const images: string[] = [];
  $("scroll-carousel div.product-gallery__media img").each((_: any, el: any) => {
...
if (largest) src = largest.split(' ')[0];
  ...
  $("div.product-form__swatches--colour label.product-form__swatch").each((_: any, el: any) => {
      const colorName = cleanText($(el).find("span.product-form__swatch-value").text());
      const swatchImg = $(el).find("img.product-form__swatch-image").attr("src");
      const isSoldOut = $(el).find("span.product-form__swatch-value--sold-out").length > 0;
      if (colorName) {
        colors.push({
          name: colorName,
  ...
  $("div.product-form__swatches--size label.product-form__swatch").each((_: any, el: any) => {
      const sizeName = cleanText($(el).find("span.product-form__swatch-value").text());
      const isSoldOut = $(el).find("span.product-form__swatch-value--sold-out").length > 0;
      if (sizeName) {
        sizes.push({
          name: sizeName,
...
if (colorName) colors.push({ name: cleanText(colorName), swatch: swatchImg, available: true });
});
...
// FIX: This function was missing from your file but scraper.ts needs it
